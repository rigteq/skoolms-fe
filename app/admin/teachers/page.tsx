...
export default function TeachersPage() {
  ...
  const filteredTeachers = useMemo(() => {
    return teachers.filter((teacher) => {
      ...
      const matchesFilter = filterSubject
        ? teacher.subject_specialization?.toLowerCase() ===
  ...
  const handleEditClick = async (id: string) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/teachers/${id}`,
        {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/teachers/${selectedTeacher.id}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 font-medium">
            Manage your school's teaching staff and specializations.
          </p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600 border-collapse">
            <thead className="bg-slate-50/50 text-slate-400 border-b border-slate-100">
              <tr>
                <th className="px-6 py-4 font-bold text-[10px] uppercase tracking-widest leading-none">
                  Teacher Details
                </th>
                <th className="px-6 py-4 font-bold text-[10px] uppercase tracking-widest leading-none">
                  Subject
                ...
                <th className="px-6 py-4 font-bold text-[10px] uppercase tracking-widest text-right leading-none">
                  Actions
                </th>
              </tr>
            </thead>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="p-2 text-slate-400 hover:text-[#3b71ca] hover:bg-blue-50 rounded-lg transition-all"
                          title="Edit"
                        >
            ...
            <form className="p-8 space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="pt-6 border-t border-slate-100 flex items-center justify-end gap-3 mt-6">
                ...
                <button
                  ...
                  className="px-8 py-3 bg-[#3b71ca] text-white font-bold text-sm rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-200/50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isProcessing && (
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-slate-100 font-sans">
            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 text-slate-800">
              <div>
                ...
                <p className="text-slate-500 text-xs mt-0.5 font-medium tracking-tight">
                  Update information for {selectedTeacher?.full_name}.
                </p>
              </div>
              <button
                onClick={() => setIsEditModalOpen(false)}
