...
export default function ClassesPage() {
    ...
    const [selectedClassItem, setSelectedClassItem] = useState<ClassType | null>(null);
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`http://localhost:5000/api/v1/classes/${selectedClassItem.id}`, {
                method: "PATCH",
                headers: {
                        ...
                        <div className="p-6">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleEditClick(cls.id)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl">
                                        <Edit className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleDeleteClick(cls)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-slate-100 font-sans">
                        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 text-slate-800">
                            <div>
                                <h2 className="text-xl font-bold tracking-tight">Edit Class Details</h2>
                                <p className="text-slate-500 text-xs mt-0.5 font-medium tracking-tight">Update information for {selectedClassItem?.class_name}.</p>
                            </div>
                            <button
                                onClick={() => setIsEditModalOpen(false)}
            ...
            {isDeleteModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-slate-100 p-8 text-center font-sans">
                        ...
                        <p className="text-slate-500 text-sm mb-8 font-medium">Are you sure you want to delete <span className="font-bold text-slate-700">{selectedClassItem?.class_name}</span>? This action cannot be undone.</p>
