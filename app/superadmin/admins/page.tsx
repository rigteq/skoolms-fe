...
export default function AdminsPage() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const result = await res.json();
      console.log("fetch admins:", result.data);
  ...
  const validateForm = () => {
    ...
    if (!form.school_id) newErrors.school_id = "School selection is required";
  ...
  const filteredAdmins = admins.filter((admin) => {
    ...
    return (
      ...
      (admin.current_address || "").includes(pageSearch) ||
      (admin.dob || "").includes(pageSearch)
    );
  });
  return (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 font-medium italic">
            Manage system administrators, their roles, and access across all
            schools.
          </p>
        </div>
        <button
                ...
                filteredAdmins.map((admin) => (
                  ...
                  >
                    <td className="px-8 py-6">
                      {admin.full_name}
                      <span className="block text-[10px] text-slate-400 mt-1 uppercase font-mono tracking-tighter">
                    ...
                    <td className="px-6 py-4 font-bold">
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex items-center text-slate-500 text-[11px]">
                          <ArrowRight className="w-3 h-3 mr-2 text-slate-300" />{" "}
                          {admin.school_name}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div
                        className={`inline-flex items-center px-3 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-widest ${admin.is_active ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full mr-2 ${admin.is_active ? "bg-emerald-500" : "bg-rose-500"}`}
                        ></span>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col text-[11px] font-bold">
                        ...
                        <span className="text-slate-300 text-[10px] mt-0.5 whitespace-nowrap">
                          Last Active:{" "}
                          {admin.last_login
                ...
                <tr>
                  <td colSpan={6} className="px-8 py-32 text-center">
                    ...
                    <p className="text-slate-400 text-sm italic">
                      Try adjusting your search filters or add a new admin.
                    </p>
                  </td>
                </tr>
              )}
          ...
          <div className="relative bg-white w-full max-w-2xl max-h-[90vh] rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300 flex flex-col">
            ...
            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 flex-shrink-0">
              <div className="flex items-center gap-4">
                ...
                <h3 className="font-extrabold text-slate-800 text-xl tracking-tight uppercase tracking-widest">
                  New Admin
                </h3>
              </div>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-1.5 md:col-span-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                    Identity Name <span className="text-rose-500">*</span>
                  </label>
                  <input
                    ref={firstInputRef}
                    type="text"
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full pl-5 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold text-slate-700 focus:ring-4 focus:ring-[#4CAF50]/10 focus:border-[#4CAF50] outline-none transition-all"
                  />
                ...
                <div className="space-y-1.5 font-bold">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 tracking-tighter">
                    Operating Hub (School)
                  </label>
      ...
      {showToast && (
        <div className="fixed bottom-10 right-10 z-[60] bg-slate-900 border border-slate-800 text-white px-8 py-5 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex items-center gap-5 animate-in slide-in-from-right duration-500">
          ...
          <div className="pr-6 border-r border-slate-800">
            <p className="font-black text-sm tracking-tight leading-none text-white uppercase">
              Registry Success
            </p>
